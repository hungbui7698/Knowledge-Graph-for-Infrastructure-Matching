...
class TransE:
    def __init__(self, kg: KnowledgeGraph,
                 ...
                 batch_size, learning_rate, n_generator, n_rank_calculator, semantic_margin):
        self.kg = kg
        ...
        self.semanticMargin=semantic_margin
        #self.entityOne=tf.placeholder(dtype=tf.int32, shape=[None]) #
        #self.entityTwo=tf.placeholder(dtype=tf.int32, shape=[None]) #
        self.train_op = None
        self.loss = None
        self.global_step = tf.Variable(initial_value=0, trainable=False, name='global_step')
        self.merge = None
        '''ops for evaluation'''
        self.eval_triple = tf.placeholder(dtype=tf.int32, shape=[4])
    ...
    def build_graph(self):
        ...
        with tf.name_scope('training'):
            distance_pos, distance_neg, entityOne, entityTwo = self.infer(self.triple_pos, self.triple_neg) #TODO semantic smoothing will be added here

            self.lossTransE = self.calculate_loss(distance_pos, distance_neg, self.margin)
            if(self.triple_pos[3]==1):
                self.lossSemantic = self.calculate_semanticLoss(entityOne, entityTwo)
            else:
                self.lossSemantic = 0
            self.loss = self.lossTransE/self.kg.n_entity+self.semanticMargin*self.lossSemantic
    ...
    def infer(self, triple_pos, triple_neg):
        with tf.name_scope('lookup'):
            ...
            #entityOne = tf.nn.embedding_lookup(self.entity_embedding,entityOne)
            #entityTwo = tf.nn.embedding_lookup(self.entity_embedding,entityTwo)

        with tf.name_scope('link'):
            distance_pos = head_pos + relation_pos - tail_pos
            distance_neg = head_neg + relation_neg - tail_neg
        return distance_pos, distance_neg, head_pos, tail_pos

    def calculate_loss(self, distance_pos, distance_neg, margin): #formul buraya eklenecek
    ...
    def calculate_semanticLoss(self,entityOne,entityTwo):
        with tf.name_scope('loss'):
            if self.score_func == 'L1':  # L1 score
                score_entityOne=tf.reduce_sum(tf.abs(entityOne), axis=1)
                score_entityTwo=tf.reduce_sum(tf.abs(entityTwo), axis=1)
            else:  # L2 score
                score_entityOne=tf.reduce_sum(tf.square(entityOne), axis=1)
                score_entityTwo=tf.reduce_sum(tf.square(entityTwo), axis=1)

            loss = tf.reduce_sum(tf.square(score_entityOne-score_entityTwo), name='semantic_loss')
        return loss
    ...
    def calculate_rank(self, in_queue, out_queue):
        ...
        with open("../DatasetOfSensors/entityNames.txt") as f:
            for line in f:
                ...
                idNums.append(str(elements[1]))
        for i in range(len(entitiesName)):
            entities[entitiesName[i]]=idNums[i]
        f.close()
        with open("../DatasetOfSensors/relation2id.txt") as f:
            for line in f:
                elements = line.split("\t")
                relationsName.append(str(elements[0]))
                relationsidNums.append(str(elements[1]))
        for i in range(len(relationsName)):
            relations[relationsName[i]]=relationsidNums[i]
        f.close()
        while True:
            ...
            if idx_predictions is None:
                ...
                head, tail, relation,category = eval_triple
                head_rank_raw = 0
                tail_rank_raw = 0
                head_rank_filter = 0
                tail_rank_filter = 0
                for candidate in idx_head_prediction[::-1]:
                    if candidate == head:
                        ...
                        head_rank_raw += 1
                        if (candidate, tail, relation,0) in self.kg.golden_triple_pool or (candidate, tail, relation,1) in self.kg.golden_triple_pool:
                            continue
                        else:
                            head_rank_filter += 1
                for candidate in idx_tail_prediction[::-1]:
                    if candidate == tail:
                        ...
                        tail_rank_raw += 1
                        if (head, candidate, relation,0) in self.kg.golden_triple_pool or (head, candidate, relation,1) in self.kg.golden_triple_pool:
                            continue
                        else:
                            tail_rank_filter += 1
                out_queue.put((head_rank_raw, tail_rank_raw, head_rank_filter, tail_rank_filter))
                in_queue.task_done()
                #print("test rank",head_rank_raw,tail_rank_raw, head_rank_filter, tail_rank_filter)
                print("triple",eval_triple)
                #print("test head",idx_head_prediction[::-1])
                print("test tail",idx_tail_prediction[::-1])
                for idx, candidate in enumerate(idx_tail_prediction[::-1]):
                    if int(relation) == int(relations["UserApp_require_Network_Slice"]):
                        if int(candidate) == int(entities["Network_Slice_Test_1"]):
                            print("Network_Slice_Test_1", idx)
                        if int(candidate) == int(entities["Network_Slice_Test_2"]):
                            print("Network_Slice_Test_2", idx)
                        if int(candidate) == int(entities["Network_Slice_Test_3"]):
                            print("Network_Slice_Test_3", idx)
                    if int(relation) == int(relations["UserApp_require_Cloud_Slice"]):
                        if int(candidate) == int(entities["Cloud_Slice_1"]):
                            print("Cloud_slice_1", idx)
                        if int(candidate) == int(entities["Cloud_Slice_2"]):
                            print("Cloud_slice_2", idx)
                        if int(candidate) == int(entities["Cloud_Slice_3"]):
                            print("Cloud_slice_3", idx)
                        if int(candidate) == int(entities["Cloud_Slice_4"]):
                            print("Cloud_slice_4", idx)
                        if int(candidate) == int(entities["Cloud_Slice_5"]):
                            print("Cloud_slice_5", idx)
