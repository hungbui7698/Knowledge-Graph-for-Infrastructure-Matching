...
class KnowledgeGraph:
    ...
    def load_dicts(self):
        ...
        category_dict_file = 'category2id.txt'
        entity_category_dict_file = 'entity_categories.txt'
        ...
        #print('-----Loading category dict-----')
        category_df = pd.read_table(os.path.join(self.data_dir, category_dict_file), header=None)
        self.category_dict= dict(zip(category_df[0], category_df[1]))
        self.n_categories = len(self.category_dict)
        #print('#categories: {}'.format(self.n_categories))

        #print('-----Loading entity_category dict-----')
        entity_category_df = pd.read_table(os.path.join(self.data_dir, entity_category_dict_file), header=None)
        self.entity_categories_dict= dict(zip(entity_category_df[0], entity_category_df[1]))
    ...
    def load_triples(self):
        ...
        for i in range(len(self.training_triples)):
            triple=self.training_triples[i]
            #print(triple)
            if (self.entity_categories_dict[triple[0]]==self.entity_categories_dict[triple[1]]):
                self.training_triples[i]=self.training_triples[i]+(1,)
            else:
                self.training_triples[i] = self.training_triples[i] + (0,)
            #print(self.training_triples[i])


        for i in range(len(self.validation_triples)):
            triple=self.validation_triples[i]
            #print(triple)
            if (self.entity_categories_dict[triple[0]]==self.entity_categories_dict[triple[1]]):
                self.validation_triples[i]=self.validation_triples[i]+(1,)
            else:
                self.validation_triples[i] = self.validation_triples[i] + (0,)
            #print(self.validation_triples[i])

        for i in range(len(self.test_triples)):
            triple=self.test_triples[i]
            #print(triple)
            if (self.entity_categories_dict[triple[0]]==self.entity_categories_dict[triple[1]]):
                self.test_triples[i]=self.test_triples[i]+(1,)
            else:
                self.test_triples[i] = self.test_triples[i] + (0,)
            #print(self.test_triples[i])
    ...
    def generate_training_batch(self, in_queue, out_queue):
        while True:
            ...
            if raw_batch is None:
                ...
                for head, tail, relation,category in batch_pos:
                    head_neg = head
                    tail_neg = tail
                    category_neg=0
                    while True:
                        ...
                        if (head_neg, tail_neg, relation) not in self.training_triple_pool:
                            break
                    if (self.entity_categories_dict[head_neg] == self.entity_categories_dict[tail_neg]):
                        category_neg=1
                    batch_neg.append((head_neg, tail_neg, relation,category_neg))
